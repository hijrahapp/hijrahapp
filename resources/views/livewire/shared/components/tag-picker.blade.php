<div>
        ...
        @if($showSuggestions && count($suggestions) > 0)
            <div class="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
                @foreach($suggestions as $s)
                    <button type="button" class="w-full text-left px-4 py-2 hover:bg-gray-100 flex items-center justify-between" wire:click="select({{ $s['id'] }}, '{{ $s['title'] }}')">
                        <span>{{ $s['title'] }}</span>
    ...
    @if(count($this->selected) > 0)
        <div class="flex flex-wrap gap-2 mt-2">
            @foreach($this->selected as $id => $title)
