    ...
    <div class="kt-modal-content max-w-[600px] top-[15%] max-h-[96vh] overflow-y-auto">
        ...
        <form wire:submit.prevent="save">
            <div class="kt-modal-body p-5 flex flex-col gap-4">
                ...
                <div class="{{ $isEditMode ? 'opacity-60' : '' }}">
                    <label class="block text-sm font-medium mb-1">Minutes <span class="text-destructive">*</span></label>
                    @if($isEditMode)
                        <input type="text" class="kt-input w-full" value="{{ $minutes }}" disabled />
                    @else
                        <input type="text" class="kt-input w-full" wire:model.defer="minutes" placeholder="Enter estimated minutes" />
                    @endif
                    @error('minutes')<span class="text-destructive text-xs">{{ $message }}</span>@enderror
                </div>
                ...
                <div class="{{ $isEditMode ? 'opacity-60 pointer-events-none' : '' }}">
                    <label class="block text-sm font-medium mb-1">Dependencies (modules this module depends on)</label>
                    <p class="text-xs text-secondary-foreground/70 mb-2">Select modules that must be completed before users can access this module.</p>
                    <div class="relative">
                        <div class="kt-input">
                            <i class="ki-filled ki-filter"></i>
                            <input type="text" class="kt-input" placeholder="Search linked modules" wire:model.live="dependencySearch" @if($isEditMode) disabled @endif />
                        </div>
