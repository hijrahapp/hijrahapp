<div class="flex items-center justify-center grow bg-center bg-no-repeat page-bg">
    <div class="kt-card max-w-[440px] w-full">
        <div class="kt-card-content p-10">
            <div class="flex justify-center mb-5">
                <img alt="image" class="dark:hidden max-h-[180px]" src="/assets/media/illustrations/done.png"/>
                <img alt="image" class="light:hidden max-h-[180px]" src="/assets/media/illustrations/done.png"/>
            </div>
            <h3 class="text-lg font-medium text-mono text-center mb-4">
                Your password has been changed successfully
            </h3>
            <div class="text-sm text-center text-secondary-foreground mb-7.5">
                Your account's security is our priority.
            </div>
            <div class="flex justify-center">
                <a class="kt-btn kt-btn-lg" href="{{ route('login') }}">
                    Login
                </a>
            </div>
        </div>
    </div>
</div>
